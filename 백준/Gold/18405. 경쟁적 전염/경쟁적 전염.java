...
public class Main {
    ...
    static List<List<Node>> virusList = new ArrayList<>(); // 1-based;
    static Node find;
    ...
    static void input() throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            for (int j = 0; j < N; j++) {
                ...
                testTube[i][j] = v;
                virusList.get(v).add(new Node(i, j));
            }
        }
        // 제한시간과 위치 입력
        st = new StringTokenizer(br.readLine());
    ...
    static void output() throws IOException {
        bw.write(String.valueOf(testTube[find.x][find.y]));
        bw.flush();
    }
    ...
    static void bfs() {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                // 위에 조건을 통과하면 감염
                testTube[nx][ny] = testTube[curr.x][curr.y]; // 숙주로 부터 새 위치 감염
                queue.offer(new Node(nx, ny, curr.time + 1)); // 새로운 바이러스 생성
            }
        }
    }

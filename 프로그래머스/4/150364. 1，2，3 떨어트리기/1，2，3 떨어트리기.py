...
def solution(edges, target):
    length = len(target) # 1-based 1..N
    # 초기화 (정렬 전)
    temp = [[] for _ in range(length + 1)]
    # 부모노드, 자식노드
    for pn, cn in edges:
        temp[pn].append(cn)
    # 트리 초기화
    tree = [deque(sorted(temp[i])) for i in range(length + 1)]
    # 리프노드 개수 세기
    goal = 0
    ...
    while goal:
        ...
        count[node] += 1
        # 충족하는지 검사
        # target은 0-based 0:1을 가리킴 node-1 필요
        # 목표로 하는 타겟에 1을 다 떨어트려도 넘는 경우 불가능
        if count[node] > target[node - 1]:
            return [-1]
        # 가능한 최솟값과 최댓값 사이에 있으면 목표를 달성한 경우
        elif count[node] <= target[node - 1] <= count[node] * 3 and not flag[node]:
            goal -= 1
            flag[node] = True
    ...
    for i in range(len(memo)):
        ...
        node = memo[i]
        # 1로 표기하는 경우
        if (count[node] - 1) <= target[node - 1] - 1 <= (count[node] - 1) * 3:
            memo[i] = 1
            target[node - 1] -= 1
        # 2로 표기하는 경우
        elif (count[node] - 1) <= target[node - 1] - 2 <= (count[node] - 1) * 3:
            memo[i] = 2
            target[node - 1] -= 2
        # 3로 표기하는 경우
        else:
